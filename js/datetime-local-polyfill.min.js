        ...
        $.fn.inputDateTimeLocal = function() {
            ...
            readDateTime = function(dt_str) {
                ...
                if (/^\d{4,}-\d\d-\d\dT\d\d:\d\d(?:\:\d\d(?:\.\d+)?)?$/.test(dt_str)) {
                    ...
                    while (millisecondPart.length < 3) { millisecondPart += '0'; }
            ...
            makeDateDisplayString = function(date_obj, elem) { var $elem, date_arr, day_names, month_names;
                $elem = $(elem);
                ...
                date_arr.push(', ');
                date_arr.push(date_obj.getFullYear().toString()); return date_arr.join(''); };
            makeTimeDisplayString = function(date_obj) {
                var ampm, time_arr;
                time_arr = new Array();
                if (date_obj.getHours() === 0) { time_arr.push('12');
                    ampm = 'AM'; } else if (date_obj.getHours() > 0 && date_obj.getHours() < 10) { time_arr.push('0');
                    time_arr.push(date_obj.getHours().toString());
                    ampm = 'AM'; } else if (date_obj.getHours() >= 10 && date_obj.getHours() < 12) { time_arr.push(date_obj.getHours().toString());
                    ampm = 'AM'; } else if (date_obj.getHours() === 12) { time_arr.push('12');
                    ampm = 'PM'; } else if (date_obj.getHours() > 12 && date_obj.getHours() < 22) { time_arr.push('0');
                    time_arr.push((date_obj.getHours() - 12).toString());
                    ampm = 'PM'; } else if (date_obj.getHours() >= 22) { time_arr.push((date_obj.getHours() - 12).toString());
                    ampm = 'PM'; }
                time_arr.push(':');
                ...
                if (date_obj.getMilliseconds() > 0) {
                    time_arr.push('.');
                    if (date_obj.getMilliseconds() % 100 === 0) { time_arr.push((date_obj.getMilliseconds() / 100).toString()); } else if (date_obj.getMilliseconds() % 10 === 0) { time_arr.push('0');
                        time_arr.push((date_obj.getMilliseconds() / 10).toString()); } else {
            ...
            increment = function(hiddenField, dateBtn, timeField, calendarDiv) {
                ...
                if (!(step != null) || step === 'any') { value.setSeconds(value.getSeconds() + 1); } else { value.setSeconds(value.getSeconds() + step); }
            ...
            stepNormalize = function(inDate, hiddenField) {
                ...
                if ((step != null) && step !== 'any') {
                    ...
                    if (min == null) { min = new Date(0); }
                    minNum = min.getTime();
                    stepDiff = (kNum - minNum) % raisedStep;
                    stepDiff2 = raisedStep - stepDiff;
                    if (stepDiff === 0) { return inDate; } else { if (stepDiff > stepDiff2) { return new Date(inDate.getTime() + stepDiff2); } else { return new Date(inDate.getTime() - stepDiff); } }
                } else { return inDate; }
            };
            $(this).filter('input[type="datetime-local"]').each(function() {
                ...
                if ((value != null) && /^\d{4,}-\d\d-\d\dT\d\d:\d\d(?:\:\d\d(?:\.\d+)?)?$/.test(value)) { value = readDateTime(value); } else { value = new Date(); }
                if (min != null) { min = readDateTime(min); if (value < min) { value.setTime(min.getTime()); } }
                if (max != null) { max = readDateTime(max); if (value > max) { value.setTime(max.getTime()); } }
                if ((step != null) && step !== 'any') { step = parseFloat(step); }
                hiddenField = document.createElement('input');
                $hiddenField = $(hiddenField);
                $hiddenField.attr({ type: "hidden", name: $this.attr('name'), value: makeDateTimeString(value) });
                $hiddenField.data({ min: min, max: max, step: step });
                ...
                if (className != null) { $calendarContainer.attr('class', className); }
                if (style != null) { $calendarContainer.attr('style', style); }
                calendarDiv = document.createElement('div');
                $calendarDiv = $(calendarDiv);
                $calendarDiv.css({ display: 'none', position: 'absolute' });
                ...
                $timeField = $(timeField);
                $timeField.attr({ type: "text", value: makeTimeDisplayString(value), size: 14 });
                ...
                $calendarDiv.datepicker({ dateFormat: 'MM dd, yy', showButtonPanel: true });
                $dateBtn.text(makeDateDisplayString(value, calendarDiv));
                if (min != null) { $calendarDiv.datepicker("option", "minDate", min); }
                ...
                if (Modernizr.csstransitions) {
                    ...
                    $dateBtn.click(function(event) { $calendarDiv.off('transitionend oTransitionEnd webkitTransitionEnd MSTransitionEnd');
                        calendarDiv.style.display = 'block';
                        calendarDiv.className = "datetime-local-calendar-dialog datetime-local-open";
                        event.preventDefault(); return false; });
                    closeFunc = function(event) {
                        var transitionend_function;
                        if (calendarDiv.className === "datetime-local-calendar-dialog datetime-local-open") { transitionend_function = function(event, ui) { calendarDiv.style.display = 'none';
                                $calendarDiv.off("transitionend oTransitionEnd webkitTransitionEnd MSTransitionEnd", transitionend_function); return null; };
                    ...
                    $dateBtn.click(function(event) { $calendarDiv.fadeIn('fast');
                        event.preventDefault(); return false; });
                    closeFunc = function(event) {
                ...
                $calendarDiv.datepicker("option", "onSelect", function(dateText, inst) {
                    ...
                    if ((min != null) && dateObj < min) { dateObj.setTime(min.getTime()); } else if ((max != null) && dateObj > max) { dateObj.setTime(max.getTime()); }
                ...
                $dateBtn.on({
                    DOMMouseScroll: function(event) {
                        if (event.originalEvent.detail < 0) { incrementDate(hiddenField, dateBtn, timeField, calendarDiv); } else { decrementDate(hiddenField, dateBtn, timeField, calendarDiv); }
                        event.preventDefault();
                        return null;
                    },
                    mousewheel: function(event) {
                        if (event.originalEvent.wheelDelta > 0) { incrementDate(hiddenField, dateBtn, timeField, calendarDiv); } else { decrementDate(hiddenField, dateBtn, timeField, calendarDiv); }
                        event.preventDefault();
                        return null;
                    },
                    keypress: function(event) {
                        if (event.keyCode === 38) { incrementDate(hiddenField, dateBtn, timeField, calendarDiv);
                            event.preventDefault(); } else if (event.keyCode === 40) { decrementDate(hiddenField, dateBtn, timeField, calendarDiv);
                            event.preventDefault(); }
                        return null;
                    }
                });
                $timeField.on({
                    DOMMouseScroll: function(event) {
                        if (event.originalEvent.detail < 0) { increment(hiddenField, dateBtn, timeField, calendarDiv); } else { decrement(hiddenField, dateBtn, timeField, calendarDiv); }
                        event.preventDefault();
                        return null;
                    },
                    mousewheel: function(event) {
                        if (event.originalEvent.wheelDelta > 0) { increment(hiddenField, dateBtn, timeField, calendarDiv); } else { decrement(hiddenField, dateBtn, timeField, calendarDiv); }
                        event.preventDefault();
                        return null;
                    },
                    keypress: function(event) {
                        var _ref, _ref1;
                        if (event.keyCode === 38) { increment(hiddenField, dateBtn, timeField, calendarDiv);
                            event.preventDefault(); } else if (event.keyCode === 40) { decrement(hiddenField, dateBtn, timeField, calendarDiv);
                            event.preventDefault(); } else if (((_ref = event.keyCode) !== 35 && _ref !== 36 && _ref !== 37 && _ref !== 39 && _ref !== 46) && ((_ref1 = event.which) !== 8 && _ref1 !== 9 && _ref1 !== 32 && _ref1 !== 45 && _ref1 !== 46 && _ref1 !== 47 && _ref1 !== 48 && _ref1 !== 49 && _ref1 !== 50 && _ref1 !== 51 && _ref1 !== 52 && _ref1 !== 53 && _ref1 !== 54 && _ref1 !== 55 && _ref1 !== 56 && _ref1 !== 57 && _ref1 !== 58 && _ref1 !== 65 && _ref1 !== 77 && _ref1 !== 80 && _ref1 !== 97 && _ref1 !== 109 && _ref1 !== 112)) { event.preventDefault(); }
                        return null;
                    },
                    change: function(event) {
                        ...
                        if (/^((?:1[0-2])|(?:0[1-9]))\:[0-5]\d(?:\:[0-5]\d(?:\.\d+)?)?\s[AaPp][Mm]$/.test($this.val())) {
                            ...
                            if (milliseconds == null) { milliseconds = 0; } else if (milliseconds.length > 3) { milliseconds = parseInt(milliseconds.substring(0, 3), 10); } else if (milliseconds.length < 3) {
                            ...
                            if (ampm === 'AM' && hours === 12) { hours = 0; } else if (ampm === 'PM' && hours !== 12) { hours += 12; }
                            dateObj.setHours(hours);
                            dateObj.setMinutes(minutes);
                            dateObj.setSeconds(seconds);
                            dateObj.setMilliseconds(milliseconds);
                            if ((min != null) && dateObj < min) { $hiddenField.val(makeDateTimeString(min)).change();
                                $this.val(makeTimeDisplayString(min)); } else if ((max != null) && dateObj > max) { $hiddenField.val(makeDateTimeString(max)).change();
                                $this.val(makeTimeDisplayString(max)); } else { dateObj = stepNormalize(dateObj, hiddenField);
                                $hiddenField.val(makeDateTimeString(dateObj)).change();
                ...
                $(upBtn).on("mousedown", function(event) { var releaseFunc, timeoutFunc;
                    increment(hiddenField, dateBtn, timeField, calendarDiv);
                    timeoutFunc = function(hiddenField, dateBtn, timeField, calendarDiv, incFunc) { incFunc(hiddenField, dateBtn, timeField, calendarDiv);
                        $(timeField).data('timeoutID', window.setTimeout(timeoutFunc, 10, hiddenField, dateBtn, timeField, calendarDiv, incFunc)); return null; };
                    releaseFunc = function(event) { window.clearTimeout($(timeField).data('timeoutID'));
                        $(document).off('mouseup', releaseFunc);
                        $(upBtn).off('mouseleave', releaseFunc); return null; };
                    $(document).on('mouseup', releaseFunc);
                    $(upBtn).on('mouseleave', releaseFunc);
                    $(timeField).data('timeoutID', window.setTimeout(timeoutFunc, 700, hiddenField, dateBtn, timeField, calendarDiv, increment)); return null; });
                $(downBtn).on("mousedown", function(event) { var releaseFunc, timeoutFunc;
                    decrement(hiddenField, dateBtn, timeField, calendarDiv);
                    timeoutFunc = function(hiddenField, dateBtn, timeField, calendarDiv, decFunc) { decFunc(hiddenField, dateBtn, timeField, calendarDiv);
                        $(timeField).data('timeoutID', window.setTimeout(timeoutFunc, 10, hiddenField, dateBtn, timeField, calendarDiv, decFunc)); return null; };
                    releaseFunc = function(event) { window.clearTimeout($(timeField).data('timeoutID'));
                        $(document).off('mouseup', releaseFunc);
                        $(downBtn).off('mouseleave', releaseFunc); return null; };
                    $(document).on('mouseup', releaseFunc);
                    $(downBtn).on('mouseleave', releaseFunc);
                    $(timeField).data('timeoutID', window.setTimeout(timeoutFunc, 700, hiddenField, dateBtn, timeField, calendarDiv, decrement)); return null; });
                return null;
            });
            return this;
        };
        $(function() {
            if (!Modernizr.inputtypes["datetime-local"]) { $('input[type="datetime-local"]').inputDateTimeLocal(); }
            return null;
        });
        return null;
    })(jQuery);
